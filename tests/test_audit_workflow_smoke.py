...
def test_index_template_batches_frontend_redraws_and_delegates_list_actions():
    ...
    assert (
        "function renderPagedFindingList(findings, listEl, pagerEl, state, emptyHTML)"
        in body
    )
    assert (
        '<div id="connFindingsPagination" class="findings-pagination hidden"></div>'
        in body
    )
    assert (
        'wireFindingPager("connFindingsPagination", connFindingPager, renderConnectFindingsPage)'
        in body
    )
    ...
    assert (
        "const pageEntries = entries.slice(startIndex, startIndex + historyPager.pageSize);"
        in body
    )
